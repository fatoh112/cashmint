-- Migration: Allow 'split' payment method in create_accounting_order without legacy payment row insertion
...
BEGIN
  ...
  IF p_status NOT IN ('pending','completed') OR p_payment_method NOT IN ('cash','card','split') OR p_order_type NOT IN ('dine_in','takeaway') THEN RAISE EXCEPTION 'Invalid order state'; END IF;
  ...
  -- DO NOT insert into public.payments if method is 'split'
  IF p_payment_method <> 'split' THEN
    INSERT INTO public.payments(store_id,order_id,method,status,amount,provider,paid_at)
    VALUES(p_store_id,v_order.id,p_payment_method,CASE WHEN p_status='completed' THEN 'paid' ELSE 'pending' END,v_order.total_amount,CASE WHEN p_payment_method='card' THEN 'stripe' END,CASE WHEN p_status='completed' THEN now() END);
  END IF;
