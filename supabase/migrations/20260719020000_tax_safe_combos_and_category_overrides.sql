-- Category defaults never overwrite an explicit product exception.
ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS accounting_group_is_override BOOLEAN NOT NULL DEFAULT false;

ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS bundle_product_id_snapshot UUID,
  ADD COLUMN IF NOT EXISTS bundle_product_name_snapshot TEXT,
  ADD COLUMN IF NOT EXISTS bundle_component_weight_snapshot NUMERIC(12,4);

CREATE OR REPLACE FUNCTION public.apply_category_accounting_group_to_products(p_category_id UUID)
RETURNS INTEGER LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_store_id UUID; v_group_id UUID; v_updated INTEGER;
BEGIN
  SELECT store_id, default_accounting_group_id INTO v_store_id, v_group_id FROM public.categories WHERE id=p_category_id;
  IF v_store_id IS NULL OR v_group_id IS NULL THEN RAISE EXCEPTION 'Category requires a default accounting group'; END IF;
  IF NOT (EXISTS (SELECT 1 FROM public.store_users WHERE store_id=v_store_id AND user_id=(SELECT auth.uid()) AND role='admin') OR (SELECT public.is_superadmin()) OR (SELECT auth.role())='service_role') THEN RAISE EXCEPTION 'Not allowed'; END IF;
  UPDATE public.products SET accounting_group_id=v_group_id
  WHERE category_id=p_category_id AND NOT accounting_group_is_override;
  GET DIAGNOSTICS v_updated=ROW_COUNT;
  RETURN v_updated;
END $$;
REVOKE ALL ON FUNCTION public.apply_category_accounting_group_to_products(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.apply_category_accounting_group_to_products(UUID) TO authenticated;

-- A sellable bundle is expanded into its components on the server. The bundle
-- price is allocated by component reference-price × configured weight, so each
-- component receives the VAT rate from its own accounting group.
