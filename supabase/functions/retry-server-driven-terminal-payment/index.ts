...
Deno.serve(async (req) => {
  ...
  try {
    const input = await req.json(); const { payment_request_id } = input; const { db, request } = await terminalPaymentContext(req, payment_request_id, input)
    ...
    const { data: reader } = await db.from('stripe_terminal_readers').select('*').eq('stripe_reader_id', request.stripe_reader_id).eq('payment_config_id', config.id).eq('is_enabled', true).maybeSingle()
    if (!reader) throw new Error('WisePOS E reader is unavailable')
    ...
    await db.from('payment_requests').update({ status:'waiting_for_card', reader_action_id:action.id ?? null, reader_action_status:action.action?.status ?? 'in_progress', process_attempt_count:Number(request.process_attempt_count||0)+1, updated_at:new Date().toISOString() }).eq('id',request.id)
    return json({ payment_request_id:request.id,status:'waiting_for_card' })
  } catch (error) { return json({ error:error instanceof Error ? error.message : 'Unable to retry payment' },400) }
})
