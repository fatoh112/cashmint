...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: reader, error: readerError } = await db.from('stripe_terminal_readers').select('*').eq('payment_config_id', config.id).eq('location_id', request.location_id).eq('is_enabled', true).order('updated_at', { ascending: false }).limit(1).maybeSingle()
    ...
    if (request.stripe_payment_intent_id) intent = await stripeRequest(`/payment_intents/${request.stripe_payment_intent_id}`, config.provider_config ?? {})
      ...
      const body = new URLSearchParams({ amount: String(amount), currency: String(config.currency ?? request.orders.currency).toLowerCase(), 'payment_method_types[]': 'card_present', capture_method: 'automatic', 'metadata[payment_request_id]': request.id, 'metadata[order_id]': request.order_id, 'metadata[store_id]': request.orders.store_id, 'metadata[location_id]': request.location_id, 'metadata[provider_type]': request.provider_type })
    ...
    await db.from('payment_requests').update({ stripe_payment_intent_id: intent.id, stripe_payment_intent_client_secret: null, stripe_reader_id: reader.stripe_reader_id, reader_action_id: action.id ?? null, reader_action_status: action.action?.status ?? 'in_progress', reader_action_type: 'process_payment_intent', status: 'waiting_for_card', started_at: new Date().toISOString(), process_attempt_count: Number(request.process_attempt_count || 0) + 1, last_reconciled_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', request.id).neq('status','succeeded')
    await db.from('stripe_terminal_readers').update({ status: fresh.status, action_status: action.action?.status ?? 'in_progress', action_type: 'process_payment_intent', last_seen_at: fresh.last_seen_at ? new Date(Number(fresh.last_seen_at)).toISOString() : null, last_synced_at: new Date().toISOString(), last_error_code: null, last_error_message: null }).eq('id', reader.id)
  ...
  } catch (error) { return json({ error: error instanceof Error ? error.message : 'Unable to start payment' }, 400) }
})
