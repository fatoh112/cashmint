...
export default function AccountantExports({ store, showNotification, isArabic }) {
  ...
  return <div className="max-w-3xl space-y-6" dir={isArabic ? 'rtl' : 'ltr'}>
    <div><h2 className="text-xl font-black text-slate-800 dark:text-white">{isArabic ? 'تصدير المحاسب' : 'Accountant Exports'}</h2><p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{isArabic ? 'تقارير المبيعات والضريبة والمدفوعات حسب الفترة.' : 'Sales, VAT, payment, and daily-closing reports for the selected period.'}</p></div>
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 grid sm:grid-cols-2 gap-4"><label className="text-xs font-bold text-slate-800 dark:text-slate-200">{isArabic ? 'من' : 'Start date'}<input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="mt-2 w-full p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:outline-none focus:border-amber-500" /></label><label className="text-xs font-bold text-slate-800 dark:text-slate-200">{isArabic ? 'إلى' : 'End date'}<input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="mt-2 w-full p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:outline-none focus:border-amber-500" /></label></div>
    <div className="grid sm:grid-cols-2 gap-3"><button disabled={disabled} onClick={salesExport} className="p-4 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm flex gap-2 justify-center transition-all disabled:opacity-40"><Download className="w-4 h-4" />Download Sales Transactions CSV</button><button disabled={disabled} onClick={vatExport} className="p-4 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm flex gap-2 justify-center transition-all disabled:opacity-40"><Download className="w-4 h-4" />Download VAT Summary CSV</button><button disabled={disabled} onClick={paymentsExport} className="p-4 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm flex gap-2 justify-center transition-all disabled:opacity-40"><FileText className="w-4 h-4" />Download Payments Summary CSV</button><button disabled={disabled} onClick={finalizeClosing} className="p-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm flex gap-2 justify-center transition-all disabled:opacity-40"><FileText className="w-4 h-4" />Finalize Daily Closing</button><button disabled={disabled} onClick={printClosing} className="p-4 rounded-xl bg-slate-800 hover:bg-slate-900 text-white font-bold text-sm flex gap-2 justify-center sm:col-span-2 transition-all disabled:opacity-40"><Printer className="w-4 h-4" />Generate Daily Closing PDF</button></div>
    {status && <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{status}</p>}
  </div>;
}
