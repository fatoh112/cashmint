...
export default function ItemsDashboard({ store, showNotification, isArabic }) {
  ...
  return (
    <div className="space-y-5 text-right font-sans" dir={isArabic ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 bg-slate-50/95 backdrop-blur-sm z-20 py-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
        ...
        <div className="flex items-center gap-3 self-end sm:self-auto relative">
          <button
            onClick={loadData}
            disabled={loading}
            className="p-2.5 bg-white border border-slate-200 text-slate-500 rounded-xl hover:bg-slate-50 active:scale-95 transition-all"
          >
          ...
          <div className="inline-flex rounded-xl shadow-sm">
            ...
            <div className="relative">
              ...
              {/* Dropdown Options */}
              {dropdownOpen && (
                <div className={`absolute ${isArabic ? 'left-0' : 'right-0'} mt-2 w-48 bg-white border border-slate-150 rounded-xl shadow-xl z-50 overflow-hidden text-right py-1.5`}>
                  <button
                    onClick={() => {
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pr-10 pl-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 text-xs font-semibold text-slate-800 placeholder-slate-350"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 border border-slate-200 rounded-xl text-xs font-bold text-slate-650 bg-white"
          >
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-all">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${item.type === 'single' ? 'bg-amber-50 text-amber-500' : 'bg-blue-50 text-blue-500'}`}>
                        {item.type === 'single' ? <Package className="w-4 h-4" /> : <Layers className="w-4 h-4" />}
                      </div>
                      <div>
                        <p className="font-bold text-slate-800">{item.name}</p>
                        <p className="text-[10px] text-slate-400 font-semibold">ID: {item.id.substring(0, 8)}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 font-mono text-slate-500">{item.sku || 'N/A'}</td>
                  <td className="p-4">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      item.type === 'single' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'
                    }`}>
                  ...
                  <td className="p-4 text-slate-450 leading-normal">
                    {item.type === 'group' ? (
                      <div>
                  ...
                  <td className="p-4">
                    <div className="flex justify-center gap-2.5">
                      <button
                        ...
                        className="w-7 h-7 rounded-lg bg-slate-100 text-slate-500 hover:bg-blue-50 hover:text-blue-600 flex items-center justify-center transition-all"
                      >
